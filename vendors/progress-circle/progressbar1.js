...
(function (root, factory) {
  ...
  var CanvasRenderer = function (element, options) {
     ...
     */
    var reqAnimationFrame1 = (function () {
      return window.requestAnimationFrame ||
          window.webkitRequestAnimationFrame ||
          window.mozRequestAnimationFrame ||
          function (callback) {
            window.setTimeout(callback, 600000 / 60);
          };
    }());
    ...
    this.animate1 = function (from, to) {
      var startTime = Date.now();

      var animation = function () {
        var process = Math.min(Date.now() - startTime, options.animate1.duration);
        var currentValue = options.easing(this, process, from, to - from, options.animate1.duration);
        this.draw(currentValue);

        //Show the number at the center of the circle
        options.onStep(from, to, currentValue);

        reqAnimationFrame1(animation);
  ...
  var pieChart1 = function (element, userOptions) {
    var defaultOptions = {
      barColor: '#ef1e25',
      trackColor: '#eff3f6',
      lineCap: 'round',
      ...
      easing: function (x, t, b, c, d) {//copy from jQuery easing animate1
        t = t / (d / 2);
        if (t < 1) {
          return c / 2 * t * t + b;
        }
    ...
    var init = function () {
      ...
      if (element.getAttribute && element.getAttribute('data-percent')) {
        ...
        if (options.animate1.enabled) {
          this.renderer.animate1(currentValue, newValue);
        } else {
  ...
  $.fn.pieChart1 = function (options) {

    //Iterate all the dom to draw the pie-charts
    return this.each(function () {
      if (!$.data(this, 'pieChart1')) {
        var userOptions = $.extend({}, options, $(this).data());
        $.data(this, 'pieChart1', new pieChart1(this, userOptions));
      }
    });
  };
